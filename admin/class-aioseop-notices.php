...
if ( ! class_exists( 'AIOSEOP_Notices' ) ) {
	...
	class AIOSEOP_Notices {
		...
		private function autoload_notice_files() {
			foreach ( new DirectoryIterator( DHSEO_PLUGIN_DIR . 'admin/display/notices/' ) as $file ) {
				...
				if ( $file->isFile() && 'php' === $extension ) {
					...
					if ( array_search( 'notice', explode( '-', str_replace( '.php', '', $filename ) ), true ) ) {
						include_once DHSEO_PLUGIN_DIR . 'admin/display/notices/' . $filename;
					}
				}
			}
		}
		 ...
		 * @since 3.0
		 *
		 * @uses DHSEO_PLUGIN_DIR . 'admin/display/notice-default.php' Template for default notices.
		 *
		 * @return void
		 */
		 ...
		 * @since 3.0
		 *
		 * @uses DHSEO_PLUGIN_DIR . 'admin/display/notice-aioseop.php' Template for notices.
		 *
		 * @return void
		 */
		...
		public function display_notice( $template ) {
			...
			foreach ( $this->active_notices as $a_notice_slug => $a_notice_time_display ) {
				...
				if ( defined( 'DISABLE_NAG_NOTICES' ) && true === DISABLE_NAG_NOTICES && ( ! in_array( $notice['class'], $important_admin_notices, true ) ) ) {
					...
					include DHSEO_PLUGIN_DIR . 'admin/display/notice-' . $template . '.php';
				}
			}
		}
		...
		public function display_remote_notice() {
			...
			foreach ( $this->remote_notices as $remote_notice_slug => $remote_notice ) {
				...
				if ( defined( 'DISABLE_NAG_NOTICES' ) && true === DISABLE_NAG_NOTICES && ( ! in_array( $remote_notice['notification_type'], $important_admin_notices, true ) ) ) {
					...
					include DHSEO_PLUGIN_DIR . 'admin/display/notice-remote.php';
				}
			}
		}
		...
		public function format_url( $url ) {
			$replace = array(
				...
				'(#aioseo-settings)'          => '?page=' . DHSEO_PLUGIN_DIRNAME . '/aioseop_class.php',
				'(#aioseo-general-settings)'  => '?page=' . DHSEO_PLUGIN_DIRNAME . '/aioseop_class.php',
				'(#aioseo-performance)'       => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_performance.php',
				'(#aioseo-sitemap)'           => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_sitemap.php',
				'(#aioseo-opengraph)'         => '?page=aiosp_opengraph',
				'(#aioseo-robots)'            => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_robots.php',
				'(#aioseo-file-editor)'       => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_file_editor.php',
				'(#aioseo-importer-exporter)' => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_importer_exporter.php',
				'(#aioseo-bad-robots)'        => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_bad_robots.php',
				'(#aioseo-feature-manager)'   => '?page=' . DHSEO_PLUGIN_DIRNAME . '/modules/aioseop_feature_manager.php',
			...
			if ( AIOSEOPPRO ) {
				...
				if ( aioseop_is_addon_allowed( 'schema_local_business' ) ) {
					$replace['(#aioseo-local-business)'] = '?page=' . DHSEO_PLUGIN_DIRNAME . '/pro/modules/class-aioseop-schema-local-business.php';
				}
			}
		 ...
		 * @since 3.0
		 *
		 * @see DHSEO_PLUGIN_DIR . 'js/admin-notice.js'
		 */
