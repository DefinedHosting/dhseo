...
require_once( DHSEO_PLUGIN_DIR . 'inc/general/aioseop-robots-meta.php' ); // Include the module base class.
...
class All_in_One_SEO_Pack extends All_in_One_SEO_Pack_Module {
	...
	public function get_preview_snippet_title() {
		...
		if ( $wp_query->is_category || $wp_query->is_tag || $wp_query->is_tax ) {
			if ( DHSEOPRO && ! empty( $_GET ) && ! empty( $_GET['taxonomy'] ) && ! empty( $_GET['tag_ID'] ) && function_exists( 'wp_get_split_terms' ) ) {
	...
	function get_post_description( $post ) {
		...
		if ( ! $description && ! empty( $aioseop_options['aiosp_generate_descriptions'] ) ) {
			...
			if ( ! $description ) {
				if ( ! DHSEOPRO || ( DHSEOPRO && apply_filters( 'aiosp_generate_descriptions_from_content', true, $post ) ) ) {
	...
	*/
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		switch ( $hook_suffix ) {
			...
			case 'post-new.php':
				...
				// No break required.
			case 'toplevel_page_' . DHSEO_PLUGIN_DIRNAME . '/aioseop_class':
				$count_chars_data['pluginDirName'] = DHSEO_PLUGIN_DIRNAME;
	...
	public function woo_upgrade_notice() {
		global $aioseop_notices;
		if ( class_exists( 'WooCommerce' ) && ! DHSEOPRO ) {
	...
	function admin_bar_menu() {
		...
		$toggle = '';
		if ( isset( $_POST['aiosp_use_original_title'] ) && isset( $_POST['aiosp_admin_bar'] ) && DHSEOPRO ) {
			$toggle = 'on';
		}
		if ( isset( $_POST['aiosp_use_original_title'] ) && ! isset( $_POST['aiosp_admin_bar'] ) && DHSEOPRO ) {
			$toggle = 'off';
		...
		if ( ( ! isset( $aioseop_options['aiosp_admin_bar'] ) && 'off' !== $toggle ) || ( ! empty( $aioseop_options['aiosp_admin_bar'] ) && 'off' !== $toggle ) || isset( $_POST['aiosp_admin_bar'] ) || true == apply_filters( 'aioseo_show_in_admin_bar', false ) ) {
			...
			if ( ! empty( $post ) ) {
				...
				if ( ! is_home() || ( ! is_front_page() && ! is_home() ) ) {
					...
					if ( DHSEOPRO && ( is_category() || is_tax() || is_tag() ) ) {
			...
			// --- removed by RC --- //
			// if ( current_user_can( 'update_plugins' ) && ! DHSEOPRO ) {
	...
	function admin_menu() {
		...
		if ( isset( $_POST ) && isset( $_POST['module'] ) && isset( $_POST['nonce-aioseop'] ) && ( 'All_in_One_SEO_Pack' == $_POST['module'] ) && wp_verify_nonce( $_POST['nonce-aioseop'], 'aioseop-nonce' ) ) {
			if ( isset( $_POST['Submit'] ) && DHSEOPRO ) {
